-- ========================================================================================================
-- BUG FIX: generate_member_program_schedule - Delete Only Pending Items
-- ========================================================================================================
-- Date: 2025-11-20
-- Issue: Function was deleting BOTH pending (NULL) AND missed (FALSE) items during regeneration
-- Fix: Change deletion filter to only delete pending (NULL) items
-- Impact: Preserves historical "missed" appointments during schedule regeneration
--
-- CRITICAL CHANGE:
--   FROM: AND COALESCE(s.completed_flag, false) = false;
--   TO:   AND s.completed_flag IS NULL;
-- ========================================================================================================
...
DECLARE
  ...
  v_deleted_pending integer := 0;  -- Renamed for clarity
BEGIN
  ...
  -- ========================================================================================================
  -- BUG FIX: Delete ONLY pending items (completed_flag IS NULL)
  -- OLD: AND COALESCE(s.completed_flag, false) = false;  -- Deleted both NULL and FALSE ❌
  -- NEW: AND s.completed_flag IS NULL;                   -- Deletes only NULL ✅
  -- ========================================================================================================
  ...
  WHERE s.member_program_item_id = i.member_program_item_id
    ...
    AND s.completed_flag IS NULL;  -- ✅ BUG FIX: Only delete pending items
  GET DIAGNOSTICS v_deleted_pending = ROW_COUNT;

  -- Generate schedules for active items with quantity > 0, excluding therapies in bucket 'No Show'
  FOR v_items IN
    SELECT
  ...
  LOOP
    ...
    FOR v_instance IN 0..(v_qty - 1) LOOP
      ...
      -- Insert occurrence idempotently; preserve completed and missed rows
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'deleted_pending', v_deleted_pending,  -- Renamed for clarity
...
SELECT 'SUCCESS: Bug fix deployed - generate_member_program_schedule now only deletes pending items' as status;

-- Verify function was updated
SELECT 
  proname as function_name,
  pg_get_functiondef(oid) LIKE '%completed_flag IS NULL%' as contains_bug_fix
FROM pg_proc
