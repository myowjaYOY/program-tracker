# Report Card Export - Architecture Diagram

## System Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                     CLIENT (Next.js React)                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │        Report Card Page                                │      │
│  │        /dashboard/report-card                          │      │
│  │                                                         │      │
│  │  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐ │      │
│  │  │   Member     │  │     MSQ      │  │  PROMIS-29  │ │      │
│  │  │   Progress   │  │  Assessment  │  │ Assessment  │ │      │
│  │  │     Tab      │  │     Tab      │  │     Tab     │ │      │
│  │  └──────────────┘  └──────────────┘  └─────────────┘ │      │
│  │                                                         │      │
│  │  ┌─────────────────────────────────────────────────┐  │      │
│  │  │         [Export Report] Button                   │  │      │
│  │  └─────────────────────────────────────────────────┘  │      │
│  └─────────────────────────┬───────────────────────────┘      │
│                              │                                    │
│                              │ Click                             │
│                              ▼                                    │
│  ┌───────────────────────────────────────────────────────┐      │
│  │        ExportReportModal                               │      │
│  │                                                         │      │
│  │  ☑ Member Progress                                     │      │
│  │  ☑ MSQ Assessment                                      │      │
│  │  ☑ PROMIS-29                                           │      │
│  │                                                         │      │
│  │  ☑ Include Charts                                      │      │
│  │  ☑ Include Interpretation                              │      │
│  │                                                         │      │
│  │  ⭕ Download    ⭕ Email                                │      │
│  │                                                         │      │
│  │      [Cancel]  [Export PDF] ────────────┐             │      │
│  └──────────────────────────────────────────┼─────────────┘      │
└─────────────────────────────────────────────┼───────────────────┘
                                               │
                                               │ POST /api/report-card/export-pdf
                                               │ { memberId, sections, options }
                                               ▼
┌─────────────────────────────────────────────────────────────────┐
│                   SERVER (Next.js API Routes)                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   /api/report-card/export-pdf/route.ts                │      │
│  │                                                         │      │
│  │   1. ✅ Authenticate User                              │      │
│  │   2. ✅ Validate Request                               │      │
│  │   3. 📊 Fetch Data ─────────────┐                     │      │
│  │   4. 🎨 Render Components       │                     │      │
│  │   5. 📄 Generate PDF            │                     │      │
│  │   6. ⬇️  Return File             │                     │      │
│  └──────────────────────────────────┼──────────────────┘      │
│                                      │                           │
│                                      │                           │
│                                      ▼                           │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   fetchReportCardData()                                │      │
│  │   src/lib/utils/report-data-fetcher.ts                │      │
│  │                                                         │      │
│  │   • Query member_progress_dashboard                   │      │
│  │   • Query msq_assessment data (Phase 2)               │      │
│  │   • Query promis_assessment data (Phase 2)            │      │
│  │                                                         │      │
│  │   Returns: { member, memberProgress, msq, promis }    │      │
│  └──────────────────────────┬────────────────────────────┘      │
│                               │                                   │
│                               ▼                                   │
└─────────────────────────────────────────────────────────────────┘
                                │
                                │ SQL Queries
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                        SUPABASE DATABASE                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────┐  │
│  │      leads       │  │  member_progress │  │  msq_surveys │  │
│  │                  │  │    _dashboard    │  │              │  │
│  └──────────────────┘  └──────────────────┘  └──────────────┘  │
│                                                                   │
│  ┌──────────────────┐  ┌──────────────────┐                     │
│  │ member_programs  │  │ promis_surveys   │                     │
│  │                  │  │                  │                     │
│  └──────────────────┘  └──────────────────┘                     │
└─────────────────────────────────────────────────────────────────┘
                                │
                                │ Return data
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│              REACT COMPONENT RENDERING (Server-Side)             │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   React.createElement()                                │      │
│  │                                                         │      │
│  │   PrintReportLayout                                    │      │
│  │   ├── PrintHeader (logo, member, date)                │      │
│  │   ├── Content:                                         │      │
│  │   │   ├── PrintMemberProgress       (if selected)     │      │
│  │   │   ├── PrintMsqAssessment        (if selected)     │      │
│  │   │   └── PrintPromisAssessment     (if selected)     │      │
│  │   └── PrintFooter (confidentiality)                   │      │
│  └──────────────────────────┬────────────────────────────┘      │
│                               │                                   │
│                               │ renderToString()                  │
│                               ▼                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   HTML String with embedded styles                    │      │
│  │                                                         │      │
│  │   <!DOCTYPE html>                                      │      │
│  │   <html>                                               │      │
│  │     <head>                                             │      │
│  │       <style>/* Print styles */</style>               │      │
│  │     </head>                                            │      │
│  │     <body>                                             │      │
│  │       <!-- Report content -->                          │      │
│  │     </body>                                            │      │
│  │   </html>                                              │      │
│  └──────────────────────────┬────────────────────────────┘      │
└─────────────────────────────┼───────────────────────────────────┘
                                │
                                │ Pass HTML
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                   PDF GENERATION (Puppeteer)                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   generatePdfFromHtml()                                │      │
│  │   src/lib/utils/pdf-generator.ts                      │      │
│  │                                                         │      │
│  │   1. 🚀 Launch headless Chrome                         │      │
│  │   2. 📄 Load HTML content                              │      │
│  │   3. ⏳ Wait for dynamic content (charts)              │      │
│  │   4. 🖨️  Generate PDF with print settings              │      │
│  │   5. 🗑️  Clean up browser instance                     │      │
│  └──────────────────────────┬────────────────────────────┘      │
│                               │                                   │
│                               ▼                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │       Puppeteer (Headless Chrome)                      │      │
│  │                                                         │      │
│  │   ┌─────────────────────────────────────────────┐     │      │
│  │   │   Browser Instance                           │     │      │
│  │   │                                              │     │      │
│  │   │   • Renders HTML/CSS                        │     │      │
│  │   │   • Executes JavaScript                     │     │      │
│  │   │   • Renders Recharts                        │     │      │
│  │   │   • Applies print styles                    │     │      │
│  │   │   • Generates 300 DPI PDF                   │     │      │
│  │   └─────────────────────────────────────────────┘     │      │
│  └──────────────────────────┬────────────────────────────┘      │
│                               │                                   │
│                               │ PDF Buffer                        │
│                               ▼                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   Binary PDF Data (Buffer)                            │      │
│  │   Size: ~2-5 MB                                        │      │
│  │   Format: application/pdf                             │      │
│  └──────────────────────────┬────────────────────────────┘      │
└─────────────────────────────┼───────────────────────────────────┘
                                │
                                │ HTTP Response
                                │ Content-Type: application/pdf
                                │ Content-Disposition: attachment
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                            CLIENT                                │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   Browser Downloads PDF                                │      │
│  │                                                         │      │
│  │   Report-Card-John-Doe-2025-01-15.pdf                 │      │
│  │   ⬇️  Saved to Downloads folder                        │      │
│  └───────────────────────────────────────────────────────┘      │
│                                                                   │
│  ┌───────────────────────────────────────────────────────┐      │
│  │   ✅ Success Toast                                     │      │
│  │   "Report exported successfully!"                      │      │
│  └───────────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────┘
```

## Component Flow Diagram

```
ExportReportModal
    │
    ├─ Collects Options
    │   ├─ Member ID
    │   ├─ Sections to include
    │   ├─ Format (PDF)
    │   └─ Delivery method
    │
    ▼
POST /api/report-card/export-pdf
    │
    ├─ Authentication Check
    │   └─ Supabase session validation
    │
    ├─ Data Fetching
    │   ├─ fetchReportCardData()
    │   │   ├─ Query leads table
    │   │   ├─ Query member_progress_dashboard
    │   │   ├─ Query msq_assessment (Phase 2)
    │   │   └─ Query promis_assessment (Phase 2)
    │   │
    │   └─ Returns ReportCardData
    │
    ├─ Component Rendering
    │   ├─ React.createElement(PrintReportLayout)
    │   │   ├─ PrintHeader
    │   │   ├─ PrintMemberProgress (if enabled)
    │   │   ├─ PrintMsqAssessment (if enabled, Phase 2)
    │   │   ├─ PrintPromisAssessment (if enabled, Phase 2)
    │   │   └─ PrintFooter
    │   │
    │   ├─ renderToString() → HTML
    │   └─ wrapHtmlForPdf() → Complete HTML doc
    │
    ├─ PDF Generation
    │   ├─ generatePdfFromHtml()
    │   │   ├─ puppeteer.launch()
    │   │   ├─ page.setContent(html)
    │   │   ├─ page.waitForTimeout(1500) // Charts
    │   │   ├─ page.pdf({ format: 'Letter' })
    │   │   └─ browser.close()
    │   │
    │   └─ Returns PDF Buffer
    │
    └─ HTTP Response
        ├─ Content-Type: application/pdf
        ├─ Content-Disposition: attachment
        └─ PDF Buffer in body
```

## Data Flow

```
User Action
    │
    ├─ 1. Select Member
    │   └─ Report Card page state: selectedMember
    │
    ├─ 2. Click [Export Report]
    │   └─ Opens ExportReportModal
    │
    ├─ 3. Configure Options
    │   ├─ Select sections (checkboxes)
    │   ├─ Choose format (radio)
    │   └─ Select delivery (radio)
    │
    ├─ 4. Click [Export PDF]
    │   └─ fetch('/api/report-card/export-pdf', { method: 'POST', ... })
    │
    ▼

Server Processing
    │
    ├─ 5. Authenticate
    │   └─ Verify Supabase session
    │
    ├─ 6. Fetch Data
    │   ├─ leads → member info
    │   ├─ member_progress_dashboard → progress data
    │   ├─ msq_* tables → assessment data (Phase 2)
    │   └─ promis_* tables → assessment data (Phase 2)
    │
    ├─ 7. Render React Components
    │   ├─ Server-side rendering with data
    │   └─ Convert to HTML string
    │
    ├─ 8. Generate PDF
    │   ├─ Launch Puppeteer
    │   ├─ Load HTML
    │   ├─ Wait for charts
    │   └─ Export as PDF
    │
    └─ 9. Return Response
        └─ Stream PDF to client
    │
    ▼

Client Response
    │
    ├─ 10. Receive PDF Blob
    │   └─ response.blob()
    │
    ├─ 11. Trigger Download
    │   ├─ Create blob URL
    │   ├─ Create <a> element
    │   ├─ Click programmatically
    │   └─ Clean up
    │
    ├─ 12. Show Success Toast
    │   └─ "Report exported successfully!"
    │
    └─ 13. Close Modal
```

## File Dependencies

```
Report Card Page
    │
    ├── Uses: ExportReportModal
    ├── Uses: useReportCardParticipants()
    └── Calls: POST /api/report-card/export-pdf

ExportReportModal
    │
    ├── Uses: exportOptionsSchema (validation)
    └── Calls: POST /api/report-card/export-pdf

API Route: /api/report-card/export-pdf
    │
    ├── Uses: createServerClient (Supabase)
    ├── Uses: fetchReportCardData()
    ├── Uses: generatePdfFromHtml()
    ├── Uses: PrintReportLayout
    ├── Uses: PrintMemberProgress
    └── Uses: renderToString (React)

fetchReportCardData()
    │
    ├── Uses: Supabase client
    └── Returns: ReportCardData

generatePdfFromHtml()
    │
    ├── Uses: puppeteer
    └── Returns: Buffer

PrintReportLayout
    │
    ├── Uses: PrintHeader
    ├── Uses: PrintFooter
    ├── Uses: printStyles
    └── Wraps: child components

PrintMemberProgress
    │
    ├── Uses: MemberProgressDashboardData (type)
    └── Uses: printStyles
```

## Technology Stack

| Layer | Technology | Purpose |
|-------|-----------|---------|
| **Frontend** | Next.js 15 + React 19 | UI and page rendering |
| **UI Components** | Material-UI v7 | Design system |
| **State Management** | React Query | Data fetching and caching |
| **API Layer** | Next.js API Routes | Server-side endpoints |
| **Authentication** | Supabase Auth | User session management |
| **Database** | Supabase (PostgreSQL) | Data storage |
| **PDF Generation** | Puppeteer | Headless Chrome for PDFs |
| **React SSR** | ReactDOMServer | Server-side rendering |
| **Validation** | Zod | Schema validation |
| **Type Safety** | TypeScript | Static typing |

## Security Flow

```
1. User Clicks Export
    │
    ▼
2. Browser sends POST request
   with session cookie
    │
    ▼
3. API Route validates session
   via Supabase
    │
    ├─ Invalid → 401 Unauthorized
    │
    └─ Valid → Continue
        │
        ▼
4. Check user has access to member
   via RLS policies
    │
    ├─ No access → 403 Forbidden
    │
    └─ Has access → Continue
        │
        ▼
5. Fetch data with RLS applied
   (Supabase automatically filters)
    │
    ▼
6. Generate PDF
   (no sensitive data in logs)
    │
    ▼
7. Stream PDF directly to user
   (no file storage)
    │
    ▼
8. Clean up resources
   (close browser, clear memory)
```

## Performance Optimization

```
Request → Response Timeline

0ms     │ User clicks Export
        │
100ms   │ Modal submits form
        │ → POST /api/report-card/export-pdf
        │
200ms   │ → Authenticate user
        │
500ms   │ → Fetch data from Supabase (3 queries)
        │
800ms   │ → Render React components to HTML
        │
1.2s    │ → Launch Puppeteer
        │
2.5s    │ → Load HTML, wait for charts
        │
4.0s    │ → Generate PDF
        │
4.5s    │ → Stream response
        │
5.0s    │ ✅ User receives PDF

Total: ~5 seconds
```

**Optimization Opportunities:**
- Cache rendered HTML for 5 minutes
- Pre-warm Puppeteer instance
- Parallel data fetching
- Background job queue for batch exports
- CDN for report assets

## Error Handling

```
┌─────────────────────────────────────────┐
│         Error Scenarios                 │
├─────────────────────────────────────────┤
│                                          │
│  1. Authentication Failure               │
│     → 401 Response                       │
│     → User sees "Unauthorized" toast     │
│                                          │
│  2. Member Not Found                     │
│     → 404 Response                       │
│     → User sees "Member not found" toast │
│                                          │
│  3. Database Query Error                 │
│     → 500 Response                       │
│     → Log error server-side              │
│     → User sees "Failed to fetch data"   │
│                                          │
│  4. Puppeteer Launch Failure             │
│     → 500 Response                       │
│     → Log error with stack trace         │
│     → User sees "PDF generation failed"  │
│                                          │
│  5. Out of Memory                        │
│     → 500 Response                       │
│     → Alert DevOps                       │
│     → User sees "Server error"           │
│                                          │
│  6. Timeout (> 30 seconds)               │
│     → 504 Response                       │
│     → User sees "Request timed out"      │
│                                          │
│  All errors:                             │
│    • Logged to console                   │
│    • Browser cleanup (finally block)     │
│    • User-friendly message               │
│    • No sensitive data exposed           │
│                                          │
└─────────────────────────────────────────┘
```

## Scalability Considerations

### Current Architecture (Phase 1)
- **Synchronous processing**: Request → Generate → Response
- **Resource usage**: ~300 MB RAM per export
- **Concurrency**: Limited by server memory
- **Estimated capacity**: ~10 concurrent exports

### Future Architecture (Phase 3+)
```
┌───────────────────────────────────────────────────┐
│                   Client                          │
└────────────────┬──────────────────────────────────┘
                  │
                  ▼
┌───────────────────────────────────────────────────┐
│            API Route (Job Creator)                │
│                                                    │
│  • Create export job                              │
│  • Return job ID immediately                      │
│  • 202 Accepted response                          │
└────────────────┬──────────────────────────────────┘
                  │
                  ▼
┌───────────────────────────────────────────────────┐
│            Job Queue (Bull/BullMQ)                │
│                                                    │
│  • Priority queue                                 │
│  • Retry logic                                    │
│  • Rate limiting                                  │
└────────────────┬──────────────────────────────────┘
                  │
                  ▼
┌───────────────────────────────────────────────────┐
│         Worker Process (Background)               │
│                                                    │
│  • Process jobs asynchronously                    │
│  • Generate PDFs                                  │
│  • Store in cloud storage (S3)                   │
│  • Send notification                              │
└────────────────┬──────────────────────────────────┘
                  │
                  ▼
┌───────────────────────────────────────────────────┐
│              Notification                         │
│                                                    │
│  • Email with download link                       │
│  • In-app notification                            │
│  • WebSocket push                                 │
└───────────────────────────────────────────────────┘
```

## Summary

This architecture leverages existing infrastructure (Puppeteer, Next.js, Supabase) to create a robust PDF export system. Key benefits:

1. **Reuses existing components** - minimal code duplication
2. **Server-side rendering** - consistent output across clients
3. **Professional quality** - 300 DPI, pixel-perfect fidelity
4. **Secure** - authentication, RLS, no file storage
5. **Extensible** - easy to add more sections
6. **Maintainable** - standard React patterns

The phased approach allows for incremental delivery while maintaining production stability.
















