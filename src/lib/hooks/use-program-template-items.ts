...
export const programTemplateItemKeys = {
  ...
  byTemplate: (templateId: number) =>
    [...programTemplateItemKeys.all, 'by-template', templateId] as const,
  detail: (id: number) =>
    [...programTemplateItemKeys.all, 'detail', id] as const,
};
...
export function useProgramTemplateItems(templateId: number) {
  return useQuery<ProgramTemplateItems[], Error>({
    ...
    queryFn: async () => {
      ...
      if (!res.ok)
        throw new Error(json.error || 'Failed to fetch program template items');
...
export function useCreateProgramTemplateItem() {
  ...
  return useMutation<
    ...
    Error,
    Partial<ProgramTemplateItems> & { program_template_id: number }
  >({
    mutationFn: async data => {
      const res = await fetch(
        `/api/program-templates/${data.program_template_id}/items`,
        {
          method: 'POST',
      ...
      const json = await res.json();
      if (!res.ok)
        throw new Error(json.error || 'Failed to create program template item');
      return json.data as ProgramTemplateItems;
    },
    onSuccess: data => {
...
export function useUpdateProgramTemplateItem() {
  ...
  return useMutation<
    ProgramTemplateItems,
    Error,
    { templateId: number; itemId: number; data: Partial<ProgramTemplateItems> }
  >({
    mutationFn: async ({ templateId, itemId, data }) => {
      const res = await fetch(
        `/api/program-templates/${templateId}/items/${itemId}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
      ...
      const json = await res.json();
      if (!res.ok)
        throw new Error(json.error || 'Failed to update program template item');
      return json.data as ProgramTemplateItems;
    },
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({
        queryKey: programTemplateItemKeys.byTemplate(variables.templateId),
      });
      queryClient.invalidateQueries({
        queryKey: programTemplateItemKeys.detail(variables.itemId),
      });
      // Also invalidate the program template to update calculated fields
      queryClient.invalidateQueries({
        queryKey: programTemplateKeys.detail(variables.templateId),
      });
      queryClient.invalidateQueries({ queryKey: programTemplateKeys.list() });
    },
  });
