...
export async function POST(
  ...
  try {
    ...
    // Validate: cannot mark as ordered if already received
    if (currentRequest.received_date) {
      return NextResponse.json(
        { error: 'Cannot mark as ordered: item already received' },
        { status: 400 }
      );
    }

    // Validate: cannot mark as ordered if cancelled
    if (currentRequest.is_cancelled) {
      return NextResponse.json(
        { error: 'Cannot mark as ordered: item request is cancelled' },
        { status: 400 }
      );
    }

    // Update the item request - set ordered_date and ordered_by
    ...
    if (error) {
      console.error('Error marking item request as ordered:', error);
      return NextResponse.json(
        { error: 'Failed to mark as ordered' },
        { status: 500 }
      );
    }
    ...
    console.error('Unexpected error in POST /api/item-requests/[id]/mark-ordered:', e);
    return NextResponse.json(
