...
export async function GET(_req: NextRequest) {
  ...
  try {
    ...
    if (statusErr) return NextResponse.json({ error: 'Failed to load statuses' }, { status: 500 });
    const activeStatus = (statuses || []).find(s => (s as any).status_name?.toLowerCase() === 'active');
    if (!activeStatus) return NextResponse.json({ error: 'Active status not found' }, { status: 500 });
    ...
    if (itemIds.length > 0) {
      ...
      scheduleIds = (sched || []).map((r: any) => r.member_program_item_schedule_id);
    }
