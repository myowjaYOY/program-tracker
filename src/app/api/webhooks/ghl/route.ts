...
import { extractContactInfo, verifyGHLWebhookSignature } from '@/lib/utils/ghl-webhook';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (signature && webhookSecret) {
      const isValid = verifyGHLWebhookSignature(rawBody, signature, webhookSecret);
      if (!isValid) {
...
export async function GET() {
  return NextResponse.json({ message: 'GHL webhook endpoint is active' }, { status: 200 });
}
