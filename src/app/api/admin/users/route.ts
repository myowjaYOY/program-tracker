...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, full_name, password, is_admin = false, is_active = false } = body;
    
    console.log('Creating user with data:', { email, full_name, is_admin, is_active });
    ...
    if (authError2) {
      ...
      return NextResponse.json({ error: authError2.message || 'Failed to create user' }, { status: 400 });
    }
    ...
    if (userError2) {
      ...
      if (userError2.code === '23505') {
        ...
        return NextResponse.json({ error: 'User creation failed due to permissions' }, { status: 500 });
      } else {
        return NextResponse.json({ error: userError2.message || 'Failed to create user record' }, { status: 500 });
      }
    }
