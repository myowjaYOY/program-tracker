  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        return NextResponse.json({ error: 'Member program not found' }, { status: 404 });
      }
      console.error('Error fetching member program:', error);
      return NextResponse.json({ error: 'Failed to fetch member program' }, { status: 500 });
    }
...
export async function PUT(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to update member program' }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (itemsError) {
      ...
      return NextResponse.json({ error: 'Failed to check program items' }, { status: 500 });
    }

    if (items && items.length > 0) {
      return NextResponse.json({ 
        error: 'Cannot delete program with active items. Remove all items first.' 
      }, { status: 400 });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to delete member program' }, { status: 500 });
    }

    return NextResponse.json({ message: 'Member program deleted successfully' });
  } catch (error) {
