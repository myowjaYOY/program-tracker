...
import { useFinancingTypes, useDeleteFinancingTypes } from '@/lib/hooks/use-financing-types';
import FinancingTypesForm from '@/components/forms/financing-types-form';

// Extended interface for table display
interface FinancingTypesEntity extends Omit<FinancingTypes, 'created_at' | 'updated_at'> {
  id: string | number;
  created_at: string;
  updated_at: string;
}
...
export default function FinancingTypesTable() {
  ...
  const renderFinancingTypesForm = ({
    ...
    const formData: Partial<FinancingTypesFormData> & { financing_type_id?: number } =
      initialValues
        ? {
            financing_type_name: initialValues.financing_type_name || '',
            financing_type_description: initialValues.financing_type_description || '',
    ...
    return (
      <FinancingTypesForm initialValues={formData} onSuccess={onClose} mode={mode} />
    );
  };

  // Transform financing types data to include id property and handle null dates
  const financingTypesWithId: FinancingTypesEntity[] = (financingTypes || []).map(type => ({
    ...type,
