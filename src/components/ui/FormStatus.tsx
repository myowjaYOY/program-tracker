 ...
 */
export default function FormStatus({ status, onClose, autoHideMs = 4000 }: FormStatusProps) {
  React.useEffect(() => {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }} role="status" aria-live={status.ok ? 'polite' : 'assertive'}>
      <Typography
        variant="body2"
        sx={{ color: status.ok ? 'success.main' : 'error.main', fontWeight: 500 }}
      >
        {status.message}
      </Typography>
      {onClose && (
