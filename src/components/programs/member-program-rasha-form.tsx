...
import {
  memberProgramRashaSchema,
  MemberProgramRashaFormData,
} from '@/lib/validations/member-program-rasha';
import { RashaList } from '@/types/database.types';
import { BaseForm } from '@/components/forms/base-form';

interface MemberProgramRashaFormProps {
  rashaItems: RashaList[];
  onSave: (data: MemberProgramRashaFormData) => void;
  onCancel: () => void;
  initialValues?: Partial<MemberProgramRashaFormData>;
  mode?: 'create' | 'edit';
}

export default function MemberProgramRashaForm({
  rashaItems,
  onSave,
  onCancel,
  initialValues,
  mode = 'create',
}: MemberProgramRashaFormProps) {
  const {
    control,
    handleSubmit,
    watch,
  ...
  } = useForm<MemberProgramRashaFormData>({
    resolver: zodResolver(memberProgramRashaSchema) as any,
    defaultValues: {
      member_program_id: initialValues?.member_program_id || 0,
  ...
  const onSubmit = (data: MemberProgramRashaFormData) => {
    onSave(data);
  };
          ...
          render={({ field }) => (
            <TextField
              ...
              helperText={
                ...
                (selectedRashaItem
                  ? `Length: ${selectedRashaItem.length}s`
                  : '')
              }
            ...
            >
              ...
              {rashaItems
                ...
                .map(rashaItem => (
                  ...
                  >
                    {rashaItem.name} (Length: {rashaItem.length}s)
                  </MenuItem>
                ))}
            </TextField>
          )}
