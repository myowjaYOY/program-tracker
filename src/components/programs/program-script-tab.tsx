...
export default function ProgramScriptTab({ program }: ProgramScriptTabProps) {
  ...
  const rows: any[] = (data as any[]).map((r: any) => ({
    ...
    therapy_name: r.therapy_name ?? r.member_program_items?.therapies?.therapy_name ?? '',
    therapy_type: r.therapy_type_name ?? r.member_program_items?.therapies?.therapytype?.therapy_type_name ?? '',
  }));
