...
interface MemberProgramPaymentFormProps {
  ...
  programType?: string | undefined; // 'one-time' or 'membership'
}
...
export default function MemberProgramPaymentForm({
  ...
  // For memberships, amount and due date are NEVER editable (generated by system)
  const canEditAmount = !isMembership && (statusName === 'quote' || statusName === 'active') && !isPaymentPaid;
  const canEditDueDate = !isMembership && !isPaymentPaid;
      ...
      <Grid size={12}>
        ...
        >
          <TextField
            ...
            helperText={errors.payment_due_date?.message || (isMembership ? 'Due date is managed by the system for memberships.' : (isPaymentPaid ? 'Due date cannot be changed for paid payments.' : ''))}
          />
          <TextField
            label="Amount"
            type="number"
