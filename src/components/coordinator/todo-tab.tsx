...
import { useCoordinatorToDo, coordinatorKeys } from '@/lib/hooks/use-coordinator';
...
export default function CoordinatorToDoTab({ memberId = null, range = 'all', start, end }: CoordinatorToDoTabProps) {
  const { data = [], isLoading, error } = useCoordinatorToDo({ memberId: memberId ?? undefined, range, start, end });
  ...
  async function toggleComplete(row: any): Promise<void> {
    try {
      ...
      if (!res.ok) return;
      qc.invalidateQueries({ queryKey: coordinatorKeys.todo(new URLSearchParams(memberId ? { memberId: String(memberId), range } : { range }).toString()) });
    } catch {}
  }
      ...
      renderCell: (params) => (
        <Box>
          <IconButton
            ...
            disabled={(params.row.program_status_name || '').toLowerCase() !== 'active'}
            onClick={() => { if ((params.row.program_status_name || '').toLowerCase() === 'active') void toggleComplete(params.row); }}
          >
